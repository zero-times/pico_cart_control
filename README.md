...
Pico 固件 `0.2.3` 在 RAM 中保留最近 `192` 条诊断记录；满后覆盖最旧记录，断电后丢失，
...
步骤见 [Pico BLE 稳定性指南](docs/PICO_BLE_STABILITY_GUIDE.md)。轮速增益保存和公共校准格式见
[Pico 校准参数保存](docs/PICO_CALIBRATION_SAVE_GUIDE.md)，拉力归零和阈值见 [Pico 拉力校准](docs/PICO_FORCE_CALIBRATION_GUIDE.md)。Android 日志可取出：
