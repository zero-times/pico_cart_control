...
PROTOCOL_VERSION = "pico-cart-ble-2026-09-10-ota"
...
OTA_STAGING_PATH = "main.py.ota"
OTA_TARGET_PATH = "main.py"
OTA_BACKUP_PATH = "main.py.bak"
OTA_MAX_BYTES = 180000
OTA_CHUNK_BYTES = 96
OTA_IDLE_TIMEOUT_MS = 30000
OTA_SESSION_MAX_MS = 900000
    ...
    def poll_lines(self):
        ...
        for ch in chunk:
            if ch == "\r" or ch == "\n":
                ...
                self.buffer += ch
                limit = OTA_LINE_MAX if self.buffer.startswith("ota ") else BLE_LINE_MAX
...
class CartController:
    ...
    def info_line(self):
        return (
            "info proto={} fw={} ota=1 uart=UART{} tx=GP{} rx=GP{} state=GP{} "
            "right_inb=GP{} front=GP{} front_active={} tow_start={} ota_max={}"
        ).format(
            PROTOCOL_VERSION,
            FIRMWARE_VERSION,
            BLE_UART_ID,
...
def parse_hex_bytes(text):
    ...
    try:
        for index in range(0, len(text), 2):
            out.append(int(text[index:index + 2], 16))
    except ValueError:
        raise ValueError("hex")
    return bytes(out)
...
class FirmwareOta:
    ...
    def status_line(self, extra=""):
        line = (
            "ok ota_status active={} size={} got={} crc={:08x} expected_crc={:08x}"
        ).format(
            bool_text(self.active),
            self.expected_size,
            self.received,
            self.crc,
            self.expected_crc,
    ...
    def abort(self, reason="abort"):
        ...
        self.reset()
        if was_active:
            self.controller.hardware_log.event("ota_abort", "reason={}".format(reason))
        return reason

    def expire_if_needed(self, now=None):
        if not self.active:
            return
        now = ticks_ms() if now is None else now
        if (ticks_diff(now, self.last_ms) > OTA_IDLE_TIMEOUT_MS or
                ticks_diff(now, self.started_ms) > OTA_SESSION_MAX_MS):
            self.abort("timeout")

    def begin(self, size, crc32):
        self.abort("restart")
        if hasattr(self.controller, "stop"):
            self.controller.stop(MODE_IDLE, "ota_begin")
        if size < 32 or size > OTA_MAX_BYTES:
            return "err ota_size"
        try:
            self.file = open(OTA_STAGING_PATH, "wb")
        except OSError as err:
            return "err ota_open {}".format(err)
        self.active = True
        self.expected_size = size
        self.expected_crc = crc32 & 0xFFFFFFFF
        self.received = 0
        self.crc = 0
        self.started_ms = ticks_ms()
        self.last_ms = self.started_ms
        self.controller.hardware_log.event(
            "ota_begin", "size={} crc={:08x}".format(size, self.expected_crc)
        )
        return self.status_line()

    def data(self, offset, payload):
        if not self.active or self.file is None:
            return "err ota_inactive"
        if offset != self.received:
            return "err ota_offset got={} want={}".format(offset, self.received)
        if not payload:
            return "err ota_empty"
        if self.received + len(payload) > self.expected_size:
            self.abort("overflow")
            return "err ota_overflow"
        try:
            self.file.write(payload)
        except OSError as err:
            self.abort("write")
            return "err ota_write {}".format(err)
        self.crc = crc32_update(self.crc, payload)
        self.received += len(payload)
        self.last_ms = ticks_ms()
        if self.received == self.expected_size or (self.received % 2048) == 0:
            return self.status_line()
        return "ok ota_data got={}".format(self.received)

    def commit(self):
        if not self.active or self.file is None:
            return "err ota_inactive"
        if self.received != self.expected_size:
            return "err ota_incomplete got={} want={}".format(self.received, self.expected_size)
        if self.crc != self.expected_crc:
            self.abort("crc")
            return "err ota_crc got={:08x} want={:08x}".format(self.crc, self.expected_crc)
        try:
            self.file.flush()
            self.file.close()
            self.file = None
            ...
            self.abort("flush")
            return "err ota_flush {}".format(err)
        try:
            with open(OTA_STAGING_PATH, "rb") as handle:
                sample = handle.read(8192)
            if b"FIRMWARE_VERSION" not in sample or b"CartController" not in sample:
                self.abort("payload")
                return "err ota_payload"
            try:
                os.remove(OTA_BACKUP_PATH)
            except OSError:
                pass
            try:
                os.rename(OTA_TARGET_PATH, OTA_BACKUP_PATH)
            except OSError:
                pass
            os.rename(OTA_STAGING_PATH, OTA_TARGET_PATH)
        except OSError as err:
            self.abort("commit")
            return "err ota_commit {}".format(err)
        self.controller.hardware_log.event(
            "ota_commit", "size={} crc={:08x}".format(self.received, self.crc)
        )
        self.reset()
        return "ok ota_reboot size={} crc={:08x}".format(self.received, self.crc)

    def handle_command(self, parts, reply):
        action = parts[1].lower() if len(parts) >= 2 else "status"
        if action == "status":
            reply(self.status_line())
            return
        if action == "abort":
            reason = self.abort("command")
            reply("ok ota_abort reason={}".format(reason))
            return
        if action == "begin" and len(parts) == 4:
            try:
                size = int(parts[2])
                crc32 = int(parts[3], 16)
            except ValueError:
                reply("err ota_usage")
                return
            reply(self.begin(size, crc32))
            return
        if action == "data" and len(parts) == 4:
            try:
                offset = int(parts[2])
                payload = parse_hex_bytes(parts[3])
            except ValueError:
                reply("err ota_hex")
                return
            reply(self.data(offset, payload))
            return
        if action == "end" and len(parts) == 2:
            result = self.commit()
            reply(result)
            if result.startswith("ok ota_reboot"):
                reply("ok ota_restarting")
                sleep_ms(120)
                try:
                    import machine as pico_machine
                    pico_machine.reset()
                    return
                except Exception as err:
                    reply("err ota_reset {}".format(err))
            return
        reply("err ota_usage")
...
class CommandInterface:
    def __init__(self, controller, ble, status_led):
        ...
        self.ota = FirmwareOta(controller)
    ...
    def help(self):
        self.reply(
            "help cmd: ver info pins status param cal status|save [motor|force] time sync MS|status stream on|off hwlog dump|clear [SEQ]|status identify [S] ota begin|data|end|abort|status tow auto manual idle stop softstop keepalive tare drive L R f [P] b [P] l [P] r [P] motor SIDE DIR [P] [MS] set NAME VALUE"
        )
        self.reply(
    ...
    def handle(self, line):
        ...
        command = parts[0].lower()
        now = ticks_ms()
        if command not in ("keepalive", "status", "hwlog", "diag", "time", "cal", "param", "ota") and (
        ...
        try:
            if command == "ota":
                self.ota.handle_command(parts, self.reply)
                return
...
def main():
    ...
    try:
        while True:
            ...
            commands.ota.expire_if_needed(now)
